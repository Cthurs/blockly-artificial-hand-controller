...
goog.DEPENDENCIES_ENABLED && (goog.dependencies_ = {
  ...
  if (goog.inHtmlDocument_()) {
    ...
    } else goog.ENABLE_CHROME_APP_SAFE_SCRIPT_LOADING ? goog.appendScriptSrcNode_(a) : goog.writeScriptSrcNode_(a); else c.write('<script type="text/javascript">' +
      b + "\x3c/script>");
    return !0
  }
  return !1
  ...
  function b(a) {
    if (!(a in e.written || a in e.visited)) {
      e.visited[a] = !0;
      if (a in e.requires)for (var f in e.requires[a])if (!goog.isProvided_(f))if (f in e.nameToPath)b(e.nameToPath[f]); else throw Error("Undefined nameToPath for " +
        f);
...
goog.loadModule = function (a) {
  ...
  try {
    ...
    var c;
    if (goog.isFunction(a))c = a.call(void 0, {}); else if (goog.isString(a))c = goog.loadModuleFromSource_.call(void 0, a); else throw Error("Invalid module definition");
...
goog.string.compareVersions = function (a, b) {
  for (var c = 0, d = goog.string.trim(String(a)).split("."), e = goog.string.trim(String(b)).split("."), f = Math.max(d.length, e.length), g = 0; 0 == c && g < f; g++) {
    ...
    do {
      ...
      var c = 0 == h[1].length ? 0 : parseInt(h[1], 10), m = 0 == k[1].length ? 0 : parseInt(k[1], 10), c = goog.string.compareElements_(c, m) || goog.string.compareElements_(0 == h[2].length, 0 == k[2].length) ||
...
goog.events.listen_ = function (a, b, c, d, e, f) {
  ...
  d.src = a;
  d.listener = c;
  if (a.addEventListener)a.addEventListener(b.toString(), d, g); else if (a.attachEvent)a.attachEvent(goog.events.getOnString_(b.toString()), d); else throw Error("addEventListener and attachEvent are unavailable.");
    ...
    if (goog.events.CAPTURE_SIMULATION_MODE == goog.events.CaptureSimulationMode.ON) {
      if (!goog.events.isMarkedIeEvent_(c)) {
        ...
        for (var f = a.type, g = c.length - 1; !d.propagationStopped_ && 0 <= g; g--) {
          d.currentTarget = c[g];
          var h =
            goog.events.fireListeners_(c[g], f, !0, d), e = e && h
        }
...
goog.style.installSafeStyleSheet = function (a, b) {
  var c = goog.dom.getDomHelper(b), d, e = c.getDocument();
...
goog.async.nextTick.getSetImmediateEmulator_ = function () {
  ...
  "undefined" === typeof a && "undefined" !== typeof window && window.postMessage && window.addEventListener && !goog.labs.userAgent.engine.isPresto() && (a = function () {
    ...
    "//" + b.location.host, a = goog.bind(function (a) {
...
goog.events.KeyHandler.prototype.handleEvent = function (a) {
  ...
  var f = c = goog.events.KeyCodes.normalizeKeyCode(c), g = b.keyIdentifier;
...
goog.ui.ControlRenderer.prototype.decorate = function (a, b) {
  ...
  var d = 0, e = this.getCssClass(), f = this.getStructuralCssClass(), g = !1, h = !1, k = !1, m = goog.array.toArray(goog.dom.classlist.get(b));
...
goog.ui.MenuItem.prototype.getCaption = function () {
  var a = this.getContent();
  if (goog.isArray(a)) {
    var b = goog.ui.MenuItem.ACCELERATOR_CLASS, c = goog.ui.MenuItem.MNEMONIC_WRAPPER_CLASS_, a = goog.array.map(a, function (a) {
...
goog.ui.tree.BaseNode.prototype.toSafeHtml = function () {
  var a = this.getTree(), b = !a.getShowLines() || a == this.getParent() && !a.getShowRootLines() ? this.config_.cssChildrenNoLines : this.config_.cssChildren, a = this.getExpanded() && this.hasChildren(), b = {
    "class": b,
  ...
  return goog.html.SafeHtml.create("div", {"class": this.config_.cssItem, id: this.getId()}, [this.getRowSafeHtml(), a])
};
...
goog.structs.Trie.prototype.getChildNode_ = function (a) {
  for (var b = this, c = 0; c < a.length; c++) {
    var d = a.charAt(c), b = b.childNodes_[d];
    if (!b)return
  }
  return b
};
...
Blockly.onMouseMove_ = function (a) {
  ...
  if (b.dragMode_ != Blockly.DRAG_NONE) {
    var c = a.clientX - b.startDragMouseX, d = a.clientY - b.startDragMouseY, e = b.startDragMetrics, f = b.startScrollX + c, g = b.startScrollY + d, f = Math.min(f, -e.contentLeft), g = Math.min(g, -e.contentTop), f = Math.max(f, e.viewWidth - e.contentLeft - e.contentWidth), g = Math.max(g, e.viewHeight - e.contentTop - e.contentHeight);
...
Blockly.Workspace.prototype.renameVariable = function (a, b) {
  var c = this.variableIndexOf(a), d = this.variableIndexOf(b);
...
Blockly.Workspace.prototype.deleteVariable = function (a) {
  ...
  if (-1 != b) {
    ...
    if (1 < c.length) {
      for (var d = 0, e; e = c[d]; d++)if ("procedures_defnoreturn" == e.type || "procedures_defreturn" == e.type) {
...
Blockly.Workspace.prototype.variableIndexOf = function (a) {
  for (var b = 0, c; c = this.variableList[b]; b++)if (Blockly.Names.equals(c, a))return b;
  return -1
};
...
Blockly.Bubble.prototype.createDom_ = function (a, b) {
  ...
  b ? (this.resizeGroup_ =
    Blockly.createSvgElement("g", {"class": this.workspace_.RTL ? "blocklyResizeSW" : "blocklyResizeSE"}, this.bubbleGroup_), c = 2 * Blockly.Bubble.BORDER_WIDTH, Blockly.createSvgElement("polygon", {points: "0,x x,x x,0".replace(/x/g, c.toString())}, this.resizeGroup_), Blockly.createSvgElement("line", {
...
Blockly.Bubble.prototype.positionBubble_ = function () {
  var a = this.anchorXY_.x, a = this.workspace_.RTL ? a - (this.relativeLeft_ + this.width_) : a + this.relativeLeft_;
...
Blockly.Bubble.prototype.renderArrow_ = function () {
  ...
  if (b == d && c == e)a.push("M " + b + "," + c); else {
    ...
    var k = Math.sin(h), m = Math.cos(h), p = this.getBubbleSize(), h = (p.width + p.height) / Blockly.Bubble.ARROW_THICKNESS, h = Math.min(h, p.width, p.height) / 4, p = 1 - Blockly.Bubble.ANCHOR_RADIUS / f, d = b +
      p * d, e = c + p * e, p = b + h * m, l = c + h * k, b = b - h * m, c = c - h * k, k = g + this.arrow_radians_;
...
Blockly.Icon.prototype.createIcon = function () {
  this.iconGroup_ || (this.iconGroup_ = Blockly.createSvgElement("g", {"class": "blocklyIconGroup"}, null), this.block_.isInFlyout && Blockly.addClass_(this.iconGroup_, "blocklyIconGroupReadonly"), this.drawIcon_(this.iconGroup_), this.block_.getSvgRoot().appendChild(this.iconGroup_), Blockly.bindEventWithChecks_(this.iconGroup_, "mouseup", this, this.iconClick_), this.updateEditable())
};
...
Blockly.Icon.prototype.computeIconLocation = function () {
  var a = this.block_.getRelativeToSurfaceXY(), b = Blockly.getRelativeXY_(this.iconGroup_), a = new goog.math.Coordinate(a.x + b.x + this.SIZE / 2, a.y + b.y + this.SIZE / 2);
...
Blockly.Comment.prototype.drawIcon_ = function (a) {
  Blockly.createSvgElement("circle", {"class": "blocklyIconShape", r: "8", cx: "8", cy: "8"}, a);
  ...
  Blockly.createSvgElement("rect", {"class": "blocklyIconSymbol", x: "6.8", y: "10.78", height: "2", width: "2"}, a)
};
...
Blockly.Connection.prototype.canConnectWithReason_ = function (a) {
  ...
  if (this.isSuperior())var b = this.sourceBlock_, c = a.getSourceBlock(); else c = this.sourceBlock_, b = a.getSourceBlock();
...
Blockly.Field.prototype.render_ = function () {
  if (this.visible_ && this.textElement_) {
    ...
    if (Blockly.Field.cacheWidths_ && Blockly.Field.cacheWidths_[a])var b = Blockly.Field.cacheWidths_[a]; else {
      try {
...
Blockly.Tooltip.show_ = function () {
  ...
  if (Blockly.Tooltip.DIV) {
    ...
    var c = Blockly.Tooltip.lastX_, c = a ? c - (Blockly.Tooltip.OFFSET_X + Blockly.Tooltip.DIV.offsetWidth) : c + Blockly.Tooltip.OFFSET_X, d = Blockly.Tooltip.lastY_ + Blockly.Tooltip.OFFSET_Y;
...
Blockly.Options = function (a) {
  var b = !!a.readOnly;
  if (b)var c = null, d = !1, e = !1, f = !1, g = !1, h = !1, k = !1; else c = Blockly.Options.parseToolboxTree(a.toolbox), d = !(!c || !c.getElementsByTagName("category").length), e = a.trashcan, void 0 === e && (e = d), f = a.collapse, void 0 === f && (f = d), g = a.comments, void 0 === g && (g = d), h = a.disable, void 0 === h && (h = d), k = a.sounds, void 0 === k && (k = !0);
  var m = !!a.rtl, p = a.horizontalLayout;
  void 0 === p && (p = !1);
  var l = a.toolboxPosition, l = "end" === l ? !1 : !0, l = p ? l ? Blockly.TOOLBOX_AT_TOP : Blockly.TOOLBOX_AT_BOTTOM : l ==
  m ? Blockly.TOOLBOX_AT_RIGHT : Blockly.TOOLBOX_AT_LEFT, n = a.scrollbars;
  void 0 === n && (n = d);
  var q = a.css;
  void 0 === q && (q = !0);
  var t = "https://blockly-demo.appspot.com/static/media/";
  a.media ? t = a.media : a.path && (t = a.path + "media/");
  var r = !a.oneBasedIndex;
  this.RTL = m;
...
Blockly.Scrollbar.prototype.createDom_ = function () {
  this.svgGroup_ = Blockly.createSvgElement("g", {"class": "blocklyScrollbar" + (this.horizontal_ ? "Horizontal" : "Vertical")}, null);
  ...
  this.svgHandle_ = Blockly.createSvgElement("rect", {"class": "blocklyScrollbarHandle", rx: a, ry: a}, this.svgGroup_);
...
Blockly.Scrollbar.prototype.onMouseDownBar_ = function (a) {
  Blockly.Touch.clearTouchIdentifier();
  this.cleanUp_();
  if (Blockly.isRightButton(a))a.stopPropagation(); else {
    var b = Blockly.mouseToSvg(a, this.workspace_.getParentSvg(), this.workspace_.getInverseScreenCTM()), b = this.horizontal_ ? b.x : b.y, c = Blockly.getSvgXY_(this.svgHandle_, this.workspace_), c = this.horizontal_ ? c.x : c.y, d = this.handlePosition_, e = .95 * this.handleLength_;
...
Blockly.Trashcan.prototype.createDom = function () {
  this.svgGroup_ = Blockly.createSvgElement("g", {"class": "blocklyTrash"}, null);
  var a = String(Math.random()).substring(2), b = Blockly.createSvgElement("clipPath", {id: "blocklyTrashBodyClipPath" + a}, this.svgGroup_);
  Blockly.createSvgElement("rect", {width: this.WIDTH_, height: this.BODY_HEIGHT_, y: this.LID_HEIGHT_}, b);
  ...
  b = Blockly.createSvgElement("clipPath", {id: "blocklyTrashLidClipPath" + a}, this.svgGroup_);
  Blockly.createSvgElement("rect", {width: this.WIDTH_, height: this.LID_HEIGHT_}, b);
...
Blockly.Trashcan.prototype.click = function () {
  var a = this.workspace_.startScrollX - this.workspace_.scrollX, b = this.workspace_.startScrollY - this.workspace_.scrollY;
...
Blockly.Xml.blockToDom = function (a, b) {
  ...
  for (var d = 0, e; e = a.inputList[d]; d++)for (var f = 0, g; g = e.fieldRow[f]; f++)if (g.name && g.EDITABLE) {
  ...
  for (d = 0; e = a.inputList[d]; d++) {
    var k;
    g = !0;
    e.type != Blockly.DUMMY_INPUT && (h = e.connection.targetBlock(), e.type == Blockly.INPUT_VALUE ? k = goog.dom.createDom("value") : e.type == Blockly.NEXT_STATEMENT && (k = goog.dom.createDom("statement")), f = e.connection.getShadowDom(),
    !f || h && h.isShadow() || k.appendChild(Blockly.Xml.cloneShadow_(f)), h && (k.appendChild(Blockly.Xml.blockToDom(h, b)), g = !1), k.setAttribute("name", e.name), g || c.appendChild(k))
  }
...
Blockly.Xml.domToWorkspace = function (a, b) {
  ...
  var c = a.childNodes.length, e = Blockly.Events.getGroup();
  e || Blockly.Events.setGroup(!0);
  for (var f = 0; f < c; f++) {
    var g = a.childNodes[f], h = g.nodeName.toLowerCase();
    if ("block" == h || "shadow" == h && !Blockly.Events.recordUndo) {
      var h = Blockly.Xml.domToBlock(g, b), k = parseInt(g.getAttribute("x"),
        10), g = parseInt(g.getAttribute("y"), 10);
...
Blockly.ZoomControls.prototype.createDom = function () {
  ...
  this.svgGroup_ = Blockly.createSvgElement("g", {"class": "blocklyZoom"}, null);
  var b = String(Math.random()).substring(2), c = Blockly.createSvgElement("clipPath", {id: "blocklyZoomoutClipPath" + b}, this.svgGroup_);
  Blockly.createSvgElement("rect", {width: 32, height: 32, y: 77}, c);
  ...
  c = Blockly.createSvgElement("clipPath", {id: "blocklyZoominClipPath" + b}, this.svgGroup_);
  Blockly.createSvgElement("rect", {width: 32, height: 32, y: 43}, c);
  ...
  e.setAttributeNS("http://www.w3.org/1999/xlink", "xlink:href", a.options.pathToMedia +
    Blockly.SPRITE.url);
  c = Blockly.createSvgElement("clipPath", {id: "blocklyZoomresetClipPath" + b}, this.svgGroup_);
  Blockly.createSvgElement("rect", {width: 32, height: 32}, c);
...
Blockly.WorkspaceSvg.prototype.createDom = function (a) {
  this.svgGroup_ = Blockly.createSvgElement("g", {"class": "blocklyWorkspace"}, null);
  ...
  this.svgBlockCanvas_ = Blockly.createSvgElement("g", {"class": "blocklyBlockCanvas"}, this.svgGroup_, this);
  this.svgBubbleCanvas_ = Blockly.createSvgElement("g", {"class": "blocklyBubbleCanvas"},
    this.svgGroup_, this);
...
Blockly.WorkspaceSvg.prototype.paste = function (a) {
  if (this.rendered && !(a.getElementsByTagName("block").length >= this.remainingCapacity())) {
    ...
    Blockly.Events.disable();
    try {
      var b = Blockly.Xml.domToBlock(a, this), c = parseInt(a.getAttribute("x"), 10), d = parseInt(a.getAttribute("y"), 10);
      if (!isNaN(c) && !isNaN(d)) {
        this.RTL && (c = -c);
        do {
          a = !1;
          ...
          if (!a)for (var k = b.getConnections_(!1),
                        f = 0, m; m = k[f]; f++)if (m.closest(Blockly.SNAP_RADIUS, new goog.math.Coordinate(c, d)).connection) {
            a = !0;
            break
          }
...
Blockly.WorkspaceSvg.prototype.onMouseWheel_ = function (a) {
  Blockly.terminateDrag_();
  var b = 0 < a.deltaY ? -1 : 1, c = Blockly.mouseToSvg(a, this.getParentSvg(), this.getInverseScreenCTM());
  this.zoom(c.x, c.y, b);
  a.preventDefault()
};
...
Blockly.WorkspaceSvg.prototype.showContextMenu_ = function (a) {
  ...
  if (!this.options.readOnly && !this.isFlyout) {
    var d = [], e = this.getTopBlocks(!0), f = Blockly.genUid(), g = {};
    ...
    if (this.options.collapse) {
      for (var h = g = !1, k = 0; k < e.length; k++)for (var m = e[k]; m;)m.isCollapsed() ? g = !0 : h = !0, m = m.getNextBlock();
      var p = function (a) {
...
Blockly.WorkspaceSvg.prototype.zoomToFit = function () {
  var a = this.getMetrics(), b = this.getBlocksBoundingBox(), c = b.width, b = b.height;
  if (c) {
...
Blockly.WorkspaceSvg.prototype.updateGridPattern_ = function () {
  if (this.options.gridPattern) {
    ...
    var a = Math.floor(this.options.gridOptions.spacing / 2) + .5, b = a - this.options.gridOptions.length / 2, c = a + this.options.gridOptions.length / 2, d = this.options.gridPattern.firstChild, e = d && d.nextSibling, a = a * this.scale, b = b * this.scale, c = c * this.scale;
...
Blockly.WorkspaceSvg.getTopLevelWorkspaceMetrics_ = function () {
  ...
  if (this.toolbox_)if (this.toolboxPosition == Blockly.TOOLBOX_AT_TOP || this.toolboxPosition == Blockly.TOOLBOX_AT_BOTTOM)a.height -= this.toolbox_.getHeight(); else if (this.toolboxPosition == Blockly.TOOLBOX_AT_LEFT || this.toolboxPosition == Blockly.TOOLBOX_AT_RIGHT)a.width -= this.toolbox_.getWidth();
  var b = Blockly.Flyout.prototype.CORNER_RADIUS - 1, c = a.width - b, d = a.height - b, e = this.getBlocksBoundingBox(), f = e.width *
    this.scale, g = e.height * this.scale, h = e.x * this.scale, k = e.y * this.scale;
...
Blockly.Mutator.prototype.drawIcon_ = function (a) {
  Blockly.createSvgElement("rect", {"class": "blocklyIconShape", rx: "4", ry: "4", height: "16", width: "16"}, a);
  Blockly.createSvgElement("path", {
    "class": "blocklyIconSymbol",
    d: "m4.203,7.296 0,1.368 -0.92,0.677 -0.11,0.41 0.9,1.559 0.41,0.11 1.043,-0.457 1.187,0.683 0.127,1.134 0.3,0.3 1.8,0 0.3,-0.299 0.127,-1.138 1.185,-0.682 1.046,0.458 0.409,-0.11 0.9,-1.559 -0.11,-0.41 -0.92,-0.677 0,-1.366 0.92,-0.677 0.11,-0.41 -0.9,-1.559 -0.409,-0.109 -1.046,0.458 -1.185,-0.682 -0.127,-1.138 -0.3,-0.299 -1.8,0 -0.3,0.3 -0.126,1.135 -1.187,0.682 -1.043,-0.457 -0.41,0.11 -0.899,1.559 0.108,0.409z"
  }, a);
  Blockly.createSvgElement("circle", {"class": "blocklyIconShape", r: "2.7", cx: "8", cy: "8"}, a)
};
...
Blockly.Mutator.prototype.resizeBubble_ = function () {
  var a = 2 * Blockly.Bubble.BORDER_WIDTH, b = this.workspace_.getCanvas().getBBox(), c;
  c = this.block_.RTL ? -b.x : b.width + b.x;
  b = b.height + 3 * a;
  if (this.workspace_.flyout_)var d = this.workspace_.flyout_.getMetrics_(), b = Math.max(b, d.contentHeight + 20);
  c += 3 * a;
...
Blockly.Warning.prototype.drawIcon_ = function (a) {
  ...
  Blockly.createSvgElement("rect", {"class": "blocklyIconSymbol", x: "7", y: "11", height: "2", width: "2"}, a)
};
Blockly.Warning.textToDom_ = function (a) {
  ...
  for (var c = 0; c < a.length; c++) {
    var d = Blockly.createSvgElement("tspan", {
      dy: "1em",
      x: Blockly.Bubble.BORDER_WIDTH
    }, b), e = document.createTextNode(a[c]);
    d.appendChild(e)
  }
  return b
};
...
Blockly.Block.prototype.setColour = function (a) {
  var b = parseFloat(a);
  if (isNaN(b))if (goog.isString(a) && a.match(/^#[0-9a-fA-F]{6}$/))this.colour_ = a; else throw"Invalid colour: " + a; else this.colour_ = Blockly.hueToRgb(b)
};
...
Blockly.Block.prototype.interpolate_ = function (a, b, c) {
  ...
  1].type, "field_")) && (g = {type: "input_dummy"}, c && (g.align = c), a.push(g));
...
Blockly.ContextMenu.show = function (a, b, c) {
  ...
  if (b.length) {
    ...
    var g = a.clientX + e.x, k = a.clientY + e.y;
...
Blockly.BlockSvg.prototype.getRelativeToSurfaceXY = function () {
  var a = 0, b = 0, c = this.getSvgRoot();
  if (c) {
    do var d = Blockly.getRelativeXY_(c), a = a + d.x, b = b + d.y, c = c.parentNode; while (c && c != this.workspace.getCanvas())
  }
  return new goog.math.Coordinate(a, b)
};
Blockly.BlockSvg.prototype.moveBy = function (a, b) {
  goog.asserts.assert(!this.parentBlock_, "Block has parent.");
  var c = new Blockly.Events.Move(this), d = this.getRelativeToSurfaceXY();
...
Blockly.BlockSvg.prototype.snapToGrid = function () {
  if (this.workspace && Blockly.dragMode_ == Blockly.DRAG_NONE && !this.getParent() && !this.isInFlyout && this.workspace.options.gridOptions && this.workspace.options.gridOptions.snap) {
    var a = this.workspace.options.gridOptions.spacing, b = a / 2, c = this.getRelativeToSurfaceXY(), d = Math.round((c.x - b) / a) * a + b - c.x, a = Math.round((c.y - b) / a) * a + b - c.y, d = Math.round(d), a = Math.round(a);
...
Blockly.BlockSvg.prototype.getBoundingRectangle = function () {
  var a = this.getRelativeToSurfaceXY(this), b = this.outputConnection ? Blockly.BlockSvg.TAB_WIDTH : 0, c = this.getHeightWidth(), d;
...
Blockly.BlockSvg.prototype.onMouseDown_ = function (a) {
  if (!this.workspace.options.readOnly)if (this.isInFlyout)"touchstart" == a.type && Blockly.isRightButton(a) && (Blockly.Flyout.blockRightClick_(a, this), a.stopPropagation(), a.preventDefault()); else {
...
Blockly.BlockSvg.prototype.showContextMenu_ = function (a) {
  if (!this.workspace.options.readOnly && this.contextMenu) {
    ...
    if (this.isDeletable() && this.isMovable() && !b.isInFlyout) {
      var d = {
        text: Blockly.Msg.DUPLICATE_BLOCK, enabled: !0, callback: function () {
          Blockly.duplicate_(b)
        }
      };
      this.getDescendants().length > this.workspace.remainingCapacity() && (d.enabled = !1);
      c.push(d);
      this.isEditable() && !this.collapsed_ && this.workspace.options.comments && (d = {enabled: !goog.userAgent.IE}, this.comment ? (d.text = Blockly.Msg.REMOVE_COMMENT,
        d.callback = function () {
      ...
      if (!this.collapsed_)for (d = 1; d < this.inputList.length; d++)if (this.inputList[d - 1].type != Blockly.NEXT_STATEMENT && this.inputList[d].type != Blockly.NEXT_STATEMENT) {
        var d = {enabled: !0}, e = this.getInputsInline();
      ...
      this.workspace.options.collapse && (this.collapsed_ ?
        (d = {enabled: !0}, d.text = Blockly.Msg.EXPAND_BLOCK, d.callback = function () {
          b.setCollapsed(!1)
      ...
      var d = this.getDescendants().length, f = this.getNextBlock();
...
Blockly.BlockSvg.prototype.onMouseMove_ = function (a) {
  if ("mousemove" == a.type && 1 >= a.clientX && 0 == a.clientY && 0 == a.button)a.stopPropagation(); else {
    var b = this.getRelativeToSurfaceXY(), c = this.workspace.moveDrag(a);
...
Blockly.BlockSvg.prototype.disposeUiEffect = function () {
  ...
  var a = Blockly.getSvgXY_(this.svgGroup_, this.workspace), b = this.svgGroup_.cloneNode(!0);
  b.translateX_ = a.x;
  b.translateY_ = a.y;
...
Blockly.BlockSvg.prototype.disconnectUiEffect = function () {
  ...
  if (!(1 > this.workspace.scale)) {
    var a = this.getHeightWidth().height, a = Math.atan(10 / a) / Math.PI * 180;
    this.RTL || (a *= -1);
...
Blockly.BlockSvg.prototype.updateColour = function () {
  if (!this.disabled) {
    ...
    if (this.isShadow())b = goog.color.lighten(b, .6), a = goog.color.rgbArrayToHex(b), this.svgPathLight_.style.display = "none", this.svgPathDark_.setAttribute("fill", a); else {
      ...
      var c = goog.color.rgbArrayToHex(goog.color.lighten(b, .3)), b = goog.color.rgbArrayToHex(goog.color.darken(b, .2));
    ...
    for (a = 0; c = this.inputList[a]; a++)for (var b = 0, d; d = c.fieldRow[b]; b++)d.setText(null)
  }
};
...
Blockly.BlockSvg.prototype.setWarningText = function (a, b) {
  ...
  var c = b || "";
  if (c)this.setWarningText.pid_[c] && (clearTimeout(this.setWarningText.pid_[c]), delete this.setWarningText.pid_[c]); else for (var d in this.setWarningText.pid_)clearTimeout(this.setWarningText.pid_[d]), delete this.setWarningText.pid_[d];
...
Blockly.BlockSvg.prototype.renderCompute_ = function (a) {
  ...
  for (var d = 0, e = 0, f = !1, g = !1, h = !1, k = void 0, m = this.getInputsInline() && !this.isCollapsed(), p = 0, l; l = b[p]; p++)if (l.isVisible()) {
    var n;
  ...
  for (a = 0; n = c[a]; a++)if (n.thicker = !1, n.type == Blockly.BlockSvg.INLINE)for (b = 0; l = n[b]; b++)if (l.type == Blockly.INPUT_VALUE) {
...
Blockly.Events.Move.prototype.run = function (a) {
  var b = Blockly.Workspace.getById(this.workspaceId), c = b.getBlockById(this.blockId);
  if (c) {
...
Blockly.Events.disableOrphans = function (a) {
  if (a.type == Blockly.Events.MOVE || a.type == Blockly.Events.CREATE) {
    Blockly.Events.disable();
    if (a = Blockly.Workspace.getById(a.workspaceId).getBlockById(a.blockId))if (a.getParent() && !a.getParent().disabled) {
      a = a.getDescendants();
...
Blockly.FieldTextInput.prototype.showEditor_ = function (a) {
  ...
  a = a || !1;
  if (!a && (goog.userAgent.MOBILE || goog.userAgent.ANDROID || goog.userAgent.IPAD))a = window.prompt(Blockly.Msg.CHANGE_VALUE_TITLE, this.text_), this.sourceBlock_ && (a = this.callValidator(a)), this.setValue(a); else {
...
Blockly.FieldTextInput.prototype.widgetDispose_ = function () {
  var a = this;
  return function () {
    var b = Blockly.FieldTextInput.htmlInput_, c = b.value;
    if (a.sourceBlock_)if (c = a.callValidator(c), null === c)c = b.defaultValue; else if (a.onFinishEditing_)a.onFinishEditing_(c);
    a.setValue(c);
...
Blockly.FieldAngle.prototype.showEditor_ = function () {
  ...
  if (a.firstChild) {
    ...
    }, a), b = Blockly.createSvgElement("circle",
      {
    ...
    this.gauge_ = Blockly.createSvgElement("path", {"class": "blocklyAngleGauge"}, a);
...
Blockly.FieldAngle.prototype.updateGraph_ = function () {
  if (this.gauge_) {
    var a = Number(this.getText()) + Blockly.FieldAngle.OFFSET, b = goog.math.toRadians(a), a = ["M ", Blockly.FieldAngle.HALF, ",", Blockly.FieldAngle.HALF], c = Blockly.FieldAngle.HALF, d = Blockly.FieldAngle.HALF;
    if (!isNaN(b)) {
      var e = goog.math.toRadians(Blockly.FieldAngle.OFFSET), f = Math.cos(e) * Blockly.FieldAngle.RADIUS, g = Math.sin(e) * -Blockly.FieldAngle.RADIUS;
...
Blockly.Variables.allUsedVariables = function (a) {
  var b;
  if (a instanceof Blockly.Block)b = a.getDescendants(); else if (a.getAllBlocks)b = a.getAllBlocks(); else throw"Not Block or Workspace: " + a;
...
Blockly.Variables.generateUniqueName = function (a) {
  ...
  if (a.length)for (var c = 1, d = 0, e = "ijkmnopqrstuvwxyzabcdefgh".charAt(d); !b;) {
    for (var f = !1, g = 0; g < a.length; g++)if (a[g].toLowerCase() == e) {
      f = !0;
      break
    }
...
Blockly.Variables.createVariable = function (a) {
  for (; ;) {
    ...
    if (b)if (-1 != a.variableIndexOf(b))window.alert(Blockly.Msg.VARIABLE_ALREADY_EXISTS.replace("%1", b.toLowerCase())); else {
      a.createVariable(b);
      break
    } else {
      b = null;
...
Blockly.FieldVariable.dropdownCreate = function () {
  var a = this.sourceBlock_ && this.sourceBlock_.workspace ? this.sourceBlock_.workspace.variableList.slice(0) : [], b = this.getText();
...
Blockly.Generator.prototype.valueToCode = function (a, b, c) {
  ...
  if (!a)return "";
  var d = !1, e = Math.floor(c), f = Math.floor(b);
...
Blockly.Procedures.flyoutCategory = function (a) {
  function b(a, b) {
    for (var d = 0; d < a.length; d++) {
      var h = a[d][0], k = a[d][1], m = goog.dom.createDom("block");
...
Blockly.Procedures.mutateCallers = function (a) {
  ...
  for (var c = 0, e; e = a[c]; c++) {
    var f = e.mutationToDom(), f = f && Blockly.Xml.domToText(f);
    e.domToMutation(d);
    var g = e.mutationToDom(), g = g && Blockly.Xml.domToText(g);
...
Blockly.FlyoutButton.prototype.createDom = function () {
  this.svgGroup_ = Blockly.createSvgElement("g", {"class": "blocklyFlyoutButton"}, this.workspace_.getCanvas());
  var a = Blockly.createSvgElement("rect", {
    ...
    x: 1,
    y: 1
  }, this.svgGroup_), b = Blockly.createSvgElement("rect", {
    "class": "blocklyFlyoutButtonBackground",
    rx: 4,
    ry: 4
  }, this.svgGroup_), c = Blockly.createSvgElement("text", {
    "class": "blocklyText",
    x: 0,
    y: 0,
    "text-anchor": "middle"
...
Blockly.Flyout.prototype.position = function () {
  if (this.isVisible()) {
    ...
    if (a) {
      var b = this.horizontalLayout_ ? a.viewWidth : this.width_, b = b - this.CORNER_RADIUS;
...
Blockly.Flyout.prototype.setBackgroundPathVertical_ = function (a, b) {
  var c = this.toolboxPosition_ == Blockly.TOOLBOX_AT_RIGHT, d = ["M " + (c ? this.width_ : 0) + ",0"];
  d.push("h", a);
...
Blockly.Flyout.prototype.wheel_ = function (a) {
  ...
  if (b) {
    ...
    var c = this.getMetrics_(), b = this.horizontalLayout_ ? c.viewLeft + b : c.viewTop + b, b = Math.min(b, this.horizontalLayout_ ? c.contentWidth - c.viewWidth : c.contentHeight - c.viewHeight), b = Math.max(b, 0);
...
Blockly.Flyout.prototype.show = function (a) {
  ...
  for (var b = [], c = [], d = this.permanentlyDisabled_.length = 0, e; e = a[d]; d++)if (e.tagName) {
...
Blockly.Flyout.prototype.isDragTowardWorkspace_ = function (a, b) {
  var c = Math.atan2(b, a) / Math.PI * 180, d = !1, e = this.dragAngleRange_;
  if (this.horizontalLayout_)this.toolboxPosition_ == Blockly.TOOLBOX_AT_TOP ? c < 90 + e && c > 90 - e && (d = !0) : c > -90 - e && c < -90 + e && (d = !0); else if (this.toolboxPosition_ == Blockly.TOOLBOX_AT_LEFT)c < e && c > -e && (d = !0); else if (c < -180 + e || c > 180 - e)d = !0;
  return d
};
...
Blockly.Flyout.prototype.placeNewBlock_ = function (a) {
  var b = this.targetWorkspace_, c = a.getSvgRoot();
  if (!c)throw"originBlock is not rendered.";
  var c = Blockly.getSvgXY_(c, b), d = this.workspace_.scrollX, e = this.workspace_.scale;
  c.x += d / e - d;
...
Blockly.Flyout.prototype.getClientRect = function () {
  if (!this.svgGroup_)return null;
  var a = this.svgGroup_.getBoundingClientRect(), b = a.left, c = a.top, d = a.width, a = a.height;
  ...
  if (this.height_ != b) {
    for (c = 0; d = a[c]; c++) {
      ...
      if (d.flyoutRect_) {
        ...
        var f = d.outputConnection ? Blockly.BlockSvg.TAB_WIDTH : 0, g = d.getRelativeToSurfaceXY();
    ...
    for (c = 0; d = a[c]; c++) {
      ...
      if (this.RTL) {
        var f = d.getRelativeToSurfaceXY().x,
          g = b / this.workspace_.scale - this.MARGIN, g = g - Blockly.BlockSvg.TAB_WIDTH;
        d.moveBy(g - f, 0)
      }
...
Blockly.Toolbox.prototype.getClientRect = function () {
  ...
  var a = this.HtmlDiv.getBoundingClientRect(), b = a.left, c = a.top, d = a.width, a = a.height;
...
Blockly.Css.inject = function (a, b) {
  if (!Blockly.Css.styleSheet_) {
    ...
    var c = c.replace(/<<<PATH>>>/g, Blockly.Css.mediaPath_), d = document.createElement("style");
...
Blockly.Css.setCursor = function (a) {
  if (Blockly.Css.currentCursor_ != a) {
    ...
    var b = "url(" + Blockly.Css.mediaPath_ + "/" + a + ".cur), auto", c = ".blocklyDraggable {\n  cursor: " + b + ";\n}\n";
...
Blockly.createDom_ = function (a, b) {
  ...
  var c = Blockly.createSvgElement("svg", {
    ...
    version: "1.1",
    "class": "blocklySvg"
  }, a), d = Blockly.createSvgElement("defs", {}, c), e = String(Math.random()).substring(2), f = Blockly.createSvgElement("filter", {id: "blocklyEmbossFilter" + e}, d);
  Blockly.createSvgElement("feGaussianBlur",
    {"in": "SourceAlpha", stdDeviation: 1, result: "blur"}, f);
  ...
  Blockly.createSvgElement("fePointLight", {x: -5E3, y: -1E4, z: 2E4}, g);
  Blockly.createSvgElement("feComposite", {"in": "specOut", in2: "SourceAlpha", operator: "in", result: "specOut"}, f);
  Blockly.createSvgElement("feComposite", {
    "in": "SourceGraphic", in2: "specOut", operator: "arithmetic", k1: 0, k2: 1, k3: 1,
    k4: 0
  }, f);
  ...
  Blockly.createSvgElement("rect", {width: 10, height: 10, fill: "#aaa"}, f);
  Blockly.createSvgElement("path", {d: "M 0 0 L 10 10 M 10 0 L 0 10", stroke: "#cc0"}, f);
  b.disabledPatternId = f.id;
  d = Blockly.createSvgElement("pattern", {id: "blocklyGridPattern" + e, patternUnits: "userSpaceOnUse"}, d);
  0 < b.gridOptions.length && 0 < b.gridOptions.spacing && (Blockly.createSvgElement("line",
    {stroke: b.gridOptions.colour}, d), 1 < b.gridOptions.length && Blockly.createSvgElement("line", {stroke: b.gridOptions.colour}, d));
  b.gridPattern = d;
  return c
};
Blockly.createMainWorkspace_ = function (a, b) {
  ...
  b.readOnly || b.hasScrollbars || c.addChangeListener(function () {
    if (Blockly.dragMode_ == Blockly.DRAG_NONE) {
      var a = c.getMetrics(), e = a.viewLeft + a.absoluteLeft, f = a.viewTop + a.absoluteTop;
      if (a.contentTop < f || a.contentTop + a.contentHeight > a.viewHeight + f || a.contentLeft < (b.RTL ? a.viewLeft : e) || a.contentLeft +
        a.contentWidth > (b.RTL ? a.viewWidth : a.viewWidth + e))for (var g = c.getTopBlocks(!1), h = 0, k; k = g[h]; h++) {
        var m = k.getRelativeToSurfaceXY(), p = k.getHeightWidth(), l = f + 25 - p.height - m.y;
        0 < l && k.moveBy(0, l);
...
Blockly.bindEventWithChecks_ = function (a, b, c, d, e) {
  var f = !1, g = function (a) {
    ...
    for (var g = 0, h; h = a[g]; g++)if (!b || Blockly.Touch.shouldHandleEvent(h))Blockly.Touch.setClientFromTouch(h), c ? d.call(c, h) : d(h), f = !0
  };
  a.addEventListener(b, g, !1);
  var h = [[a, b, g]];
  if (b in Blockly.Touch.TOUCH_MAP)for (var k = function (a) {
    g(a);
    f && a.preventDefault()
...
Blockly.svgResize = function (a) {
  ...
  if (c) {
    var d = c.offsetWidth, c = c.offsetHeight;
...
Blockly.onKeyDown_ = function (a) {
  if (!Blockly.mainWorkspace.options.readOnly && !Blockly.isTargetInput_(a)) {
    var b = !1;
    if (27 == a.keyCode)Blockly.hideChaff(); else if (8 == a.keyCode || 46 == a.keyCode)a.preventDefault(), Blockly.selected && Blockly.selected.isDeletable() && (b = !0); else if (a.altKey || a.ctrlKey || a.metaKey)Blockly.selected && Blockly.selected.isDeletable() && Blockly.selected.isMovable() && (67 == a.keyCode ? (Blockly.hideChaff(), Blockly.copy_(Blockly.selected)) : 88 == a.keyCode && (Blockly.copy_(Blockly.selected), b = !0)),
